...
def apply_training_mask_strategy(rows: list[dict], strategy: str, seed: int,
                                 missing_fraction: float = 0.25) -> dict:
    """Apply a pre-registered training-only mask perturbation in place."""
    original = np.asarray([row["masks"] for row in rows], dtype=np.float32)
    ...
    if strategy == "task_aware":
        ...
        modified.fill(0.0)
        for task_index in range(original.shape[1]):
            count = int(original[:, task_index].sum())
            selected = rng.choice(len(rows), size=count, replace=False)
            modified[selected, task_index] = 1.0
    elif strategy == "shuffled_rows":
        modified = original[rng.permutation(len(rows))].copy()
    elif strategy == "artificial_missing":
        if not 0.0 < missing_fraction < 1.0:
            raise ValueError("artificial_missing_fraction must be between 0 and 1")
        for task_index in range(original.shape[1]):
            for label_value in (0.0, 1.0):
                candidates = np.flatnonzero((original[:, task_index] == 1.0) &
                                             (labels[:, task_index] == label_value))
                drop_count = int(round(len(candidates) * missing_fraction))
                if drop_count:
                    dropped = rng.choice(candidates, size=drop_count, replace=False)
                    modified[dropped, task_index] = 0.0
    else:
        raise ValueError(f"unsupported mask_strategy: {strategy}")

    for row, mask in zip(rows, modified.tolist()):
        row["masks"] = mask

    originally_unknown_selected = ((original == 0.0) & (modified == 1.0)).sum(0)
    originally_known_retained = ((original == 1.0) & (modified == 1.0)).sum(0)
    return {
        "strategy": strategy,
        "seed": seed,
        "artificial_missing_fraction": missing_fraction if strategy == "artificial_missing" else None,
        "original_known_positions_per_task": original.sum(0).astype(int).tolist(),
        "effective_positions_per_task": modified.sum(0).astype(int).tolist(),
        "originally_known_retained_per_task": originally_known_retained.astype(int).tolist(),
        "originally_unknown_selected_as_zero_control_per_task": originally_unknown_selected.astype(int).tolist(),
        "preserves_only_verified_targets": strategy in {"task_aware", "artificial_missing"},
    }
...
def main() -> None:
    ...
    mask_strategy = config.get("mask_strategy", "task_aware")
    mask_audit = apply_training_mask_strategy(
        train_rows, mask_strategy, config["seed"],
        float(config.get("artificial_missing_fraction", 0.25)))
